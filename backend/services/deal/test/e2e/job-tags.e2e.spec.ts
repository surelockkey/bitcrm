...
describe('Job Tags E2E', () => {
  ...
  it('creates, lists and reads a job tag as admin', async () => {
    const created = await request(app.getHttpServer())
      .post(BASE).set(...hdr(admin)).send({ name: 'Rush', priority: 3 })
      .expect(201);
    expect(created.body.data).toMatchObject({ name: 'Rush', priority: 3, active: true });
  ...
  it('rejects a duplicate name (409)', async () => {
    await request(app.getHttpServer()).post(BASE).set(...hdr(admin)).send({ name: 'Repeat' }).expect(201);
    await request(app.getHttpServer()).post(BASE).set(...hdr(admin)).send({ name: '  repeat ' }).expect(409);
  });

  it('updates a job tag', async () => {
    const created = await request(app.getHttpServer()).post(BASE).set(...hdr(admin)).send({ name: 'VIP' }).expect(201);
  ...
  it('hard-deletes an unreferenced job tag', async () => {
    const created = await request(app.getHttpServer()).post(BASE).set(...hdr(admin)).send({ name: 'Temp Tag' }).expect(201);
    const res = await request(app.getHttpServer())
