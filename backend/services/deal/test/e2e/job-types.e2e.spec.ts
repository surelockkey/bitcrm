...
const BASE = '/api/deals/job-types';
...
describe('Job Types E2E', () => {
  ...
  it('creates, lists and reads a job type as admin', async () => {
    const created = await request(app.getHttpServer())
      .post(BASE).set(...hdr(admin)).send({ name: 'Gate Repair', priority: 3 })
      .expect(201);
    expect(created.body.data).toMatchObject({ name: 'Gate Repair', priority: 3, active: true });
  ...
  it('rejects a duplicate name (409)', async () => {
    await request(app.getHttpServer()).post(BASE).set(...hdr(admin)).send({ name: 'Rekey' }).expect(201);
    await request(app.getHttpServer()).post(BASE).set(...hdr(admin)).send({ name: '  rekey ' }).expect(409);
  });

  it('updates a job type', async () => {
    const created = await request(app.getHttpServer()).post(BASE).set(...hdr(admin)).send({ name: 'Safe' }).expect(201);
  ...
  it('hard-deletes an unreferenced job type', async () => {
    const created = await request(app.getHttpServer()).post(BASE).set(...hdr(admin)).send({ name: 'Temp' }).expect(201);
    const res = await request(app.getHttpServer())
