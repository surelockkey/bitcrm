...
const BASE = '/api/deals/custom-fields';
...
describe('Custom Fields E2E', () => {
  ...
  it('creates, lists and reads a custom field as admin', async () => {
    const created = await request(app.getHttpServer())
      .post(BASE).set(...hdr(admin)).send({ name: 'Gate Code', type: 'text', group: 'Access', priority: 3 })
      .expect(201);
    expect(created.body.data).toMatchObject({ name: 'Gate Code', type: 'text', group: 'Access', priority: 3, active: true });
  ...
  it('rejects a duplicate name (409)', async () => {
    await request(app.getHttpServer()).post(BASE).set(...hdr(admin)).send({ name: 'Lock Brand', type: 'text' }).expect(201);
    await request(app.getHttpServer()).post(BASE).set(...hdr(admin)).send({ name: '  lock brand ', type: 'text' }).expect(409);
  });

  it('rejects a dropdown field without options (400)', async () => {
    await request(app.getHttpServer()).post(BASE).set(...hdr(admin)).send({ name: 'Brand', type: 'dropdown' }).expect(400);
  });

  it('creates a dropdown field with options', async () => {
    const created = await request(app.getHttpServer())
      .post(BASE).set(...hdr(admin)).send({ name: 'Brand', type: 'dropdown', options: ['Kwikset', 'Schlage'] })
      .expect(201);
    expect(created.body.data.options).toEqual(['Kwikset', 'Schlage']);
  });

  it('updates a custom field', async () => {
    const created = await request(app.getHttpServer()).post(BASE).set(...hdr(admin)).send({ name: 'Safe Combo', type: 'text' }).expect(201);
  ...
  it('hard-deletes an unreferenced custom field', async () => {
    const created = await request(app.getHttpServer()).post(BASE).set(...hdr(admin)).send({ name: 'Temp', type: 'text' }).expect(201);
    const res = await request(app.getHttpServer())
  ...
  it('enforces permissions: read-only cannot create', async () => {
    await request(app.getHttpServer()).post(BASE).set(...hdr(readOnly)).send({ name: 'Nope', type: 'text' }).expect(403);
  });
