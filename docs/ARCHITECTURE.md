...
`env.apiBaseUrl` у застосунку вже вказує на `…/api` (`src/lib/env.ts:5`), тож
...
ретраїти PUT з тим самим `uploadUrl`, доки він живий, а при протуханні —
перевикликати presign.
...
CREATE TABLE outbox (
  ...
  kind          TEXT NOT NULL,      -- 'confirm' | 'arrived' | 'status' | 'note' | 'photo'
  ...
  last_error    TEXT,
  state         TEXT NOT NULL       -- 'pending' | 'sending' | 'failed' | 'done'
);
...
`tech/arrived` ідемпотентні на сервері за визначенням
(`TECH/backend/services/deal/src/deals/deals.controller.ts:184-189`, `:206-208`),
повідомлення дедуплікуються за `clientMessageId`
(`BE/.../outbound/dto/send-message.dto.ts:80-86`). Тобто повторна відправка після
обриву не створює дублів. Для `notes` і `status` серверної ідемпотентності
**немає** — там ключ у нашій черзі захищає лише від подвійної відправки з нашого
боку, тому `state: 'sending'` треба фіксувати **до** запиту, а не після.
...
URL протух (S3 віддав 403) — обнуляємо `upload_url` і починаємо крок заново.
...
застосунку, а фон — це бонус, який скорочує затримку. Для великих фото варто
подивитись на `expo-file-system` `createUploadTask` з
`uploadType: BINARY_CONTENT` — він переживає згортання застосунку краще, ніж
`fetch` з тілом.

Тести: чистий редʼюсер черги (перехід станів, backoff, upsert), фейковий
транспорт, фейковий годинник. Ніяких мережевих тестів.
...
| `401` | Протух idToken | Один прозорий `POST /users/auth/refresh`, потім повтор; при повторному 401 — вихід із сесії (`setUnauthorizedHandler`, `src/lib/api/http.ts:19-21`) |
...
має бути видимим на екрані роботи (значок «не надіслано» + тап «спробувати ще»).
Черга без UI — це втрачені дані, про які ніхто не дізнається.
...
- Дії: «Підтвердив», «Прибув» (з `expo-location`, timeout 8 с, без фікса теж працює), зміна статусу — 1 дн
