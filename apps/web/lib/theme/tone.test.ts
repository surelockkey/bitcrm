...
describe("tones", () => {
  ...
  it("builds a tinted pill from the tone's own tokens", () => {
    expect(toneClasses("warning")).toBe(
  ...
  it("needs no dark: variant — the tokens already differ per theme", () => {
    for (const tone of TONES) {
      expect(toneClasses(tone), tone).not.toContain("dark:");
    }
  });
  ...
  it("is a total function over the Tone type", () => {
    for (const tone of TONES) {
      expect(typeof toneClasses(tone as Tone)).toBe("string");
    }
  });
});
