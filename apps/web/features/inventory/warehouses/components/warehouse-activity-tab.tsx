...
import { useWarehouseTransfers } from "../hooks";
import { transferDirection, transferUnits } from "../lib";

function titleFor(t: Transfer, warehouseId: string): string {
  const dir = transferDirection(t, warehouseId);
  const units = transferUnits(t);
  const u = `${units} ${units === 1 ? "unit" : "units"}`;
  if (t.type === TransferType.RECEIVE) return `Received ${u} from supplier`;
  if (dir === "in") return `Returned ${u} from a container`;
  return `Transferred ${u} to a container`;
}

export function WarehouseActivityTab({ warehouseId }: { warehouseId: string }) {
  const query = useWarehouseTransfers(warehouseId);
      ...
      <div className="flex flex-col items-center justify-center gap-3 rounded-lg border border-dashed py-14 text-center">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Receiving stock or transferring it out will show up here.
          </p>
        </div>
      </div>
    );
  ...
  return (
    <div className="space-y-1">
      <div className="divide-y rounded-lg border">
        {transfers.map((t) => {
          const inbound = transferDirection(t, warehouseId) === "in";
          return (
            <div key={t.id} className="flex items-start gap-3 px-4 py-3">
              ...
              <div className="min-w-0 flex-1">
                <div className="text-sm font-medium">{titleFor(t, warehouseId)}</div>
